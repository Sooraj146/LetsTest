...
async function loadLeaderboard() {
    ...
    try {
        ...
        thHtml += `<th class="px-8 py-5 text-right">Time Taken</th></tr>`;
        ...
        list.innerHTML = leaderboard.map(u => `
            <tr class="group hover:bg-white/[0.02] transition-all border-b border-white/[0.02]">
                ...
                <td class="px-8 py-6 text-right text-slate-400 font-mono text-xs">${formatDuration(u.durationMs)}</td>
            </tr>
        `).join('');
...
function downloadCSV() {
    ...
    const rows = data.map(u => {
        ...
        const endData = [u.totalScore, window.totalQuestions, formatDuration(u.durationMs)];
